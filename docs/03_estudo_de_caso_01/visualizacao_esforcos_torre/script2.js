...
// State
let selectedVars = ['My', 'Fy']; // Default selection: one moment, one force for comparison

// DOM Elements
const momentsCheckboxes = document.getElementById('momentsCheckboxes');
const forcesCheckboxes = document.getElementById('forcesCheckboxes');
...
function renderCheckboxes() {
    renderGroup(CATEGORIES.moments, momentsCheckboxes);
    renderGroup(CATEGORIES.forces, forcesCheckboxes);
}

function renderGroup(vars, container) {
    container.innerHTML = '';
    vars.forEach(v => {
...
function updateAnalysis() {
    ...
    // Filter allVars to only include selected ones, preserving order
    const orderedSelected = allVars.filter(v => selectedVars.includes(v));

    orderedSelected.forEach(v => {
...
function updatePlot() {
    ...
    selectedVars.forEach(v => {
        ...
        // Determine if it's a Moment or Force
        const isMoment = CATEGORIES.moments.includes(v);
        const yAxis = isMoment ? 'y' : 'y2';
    ...
    const layout = {
        title: {
            text: 'Comparação: Momentos (Eixo Esq.) vs Forças (Eixo Dir.)',
        ...
        yaxis2: {
            title: 'Forças (kN)',
            titlefont: { color: '#FFCC5C' },
            tickfont: { color: '#FFCC5C' },
            overlaying: 'y',
            side: 'right',
            gridcolor: '#333', // Optional: might clutter if both have grids
            showgrid: false // Disable grid for secondary axis to avoid clutter
        },
        legend: {
            font: { color: '#e0e0e0' },
            orientation: 'h',
        ...
        margin: { t: 60, l: 60, r: 60, b: 50 } // Increased right margin for secondary axis
    };
